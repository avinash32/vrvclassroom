...
window.todo = window.todo || {};
	...
	document.addEventListener("DOMContentLoaded", function(event) {
		...
		preloadedItems.map(function(item)
			{
				todo.todoitems.timeForNewToDoItem(item);
			});
			(function(){
			...
			ulElement.addEventListener("click",todo.todoitems.assignListeners);
	...
	check = function(event){
		...
		if(event.keyCode === 13){
			...
			todo.todoitems.timeForNewToDoItem(newItem);
		}
	}
	...
	function clearScreen(){
		var id = "clearScreen";
		todo.todoitem.removeElements(id);
	}
