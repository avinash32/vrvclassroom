// does all the assignments and checking functions
	...
	document.addEventListener("DOMContentLoaded", function(event) {
		...
		preloadedItems.map(function(item)
			{
				window.todo.todoitems.timeForNewToDoItem(item);
			});
			(function(){
			...
			ulElement.addEventListener("click",window.todo.todoitems.assignListeners);
	...
	check = function(event){
		...
		if(event.keyCode === 13){
			...
			window.todo.todoitems.timeForNewToDoItem(newItem);
		}
	}
	...
	function clearScreen(){
		var id = "clearScreen";
		window.todo.todoitem.removeElements(id);
	}
